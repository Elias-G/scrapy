...
class RequestTest(unittest.TestCase):
    ...
    def test_ajax_url(self):
        # ascii url
        r = self.request_class(url="http://www.example.com/ajax.html#!key=value", encode_url = True)
        self.assertEqual(r.url, "http://www.example.com/ajax.html?_escaped_fragment_=key%3Dvalue")
        # unicode url
        r = self.request_class(url=u"http://www.example.com/ajax.html#!key=value", encode_url = True)
        self.assertEqual(r.url, "http://www.example.com/ajax.html?_escaped_fragment_=key%3Dvalue")

    def test_ajax_url_noescape(self):
        # ascii url
        r = self.request_class(url="http://www.example.com/ajax.html#!key=value", encode_url = False)
        self.assertEqual(r.url, "http://www.example.com/ajax.html#!key=value")
        # unicode url
        r = self.request_class(url=u"http://www.example.com/ajax.html#!key=value", encode_url = False)
        self.assertEqual(r.url, "http://www.example.com/ajax.html#!key=value")

    def test_url_escaping(self):
        r = self.request_class(url=u'http://www.scrapy.org/price/ "',encode_url=True)
        self.assertEqual(r.url, "http://www.scrapy.org/price/%20%22")

    def test_url_noescape(self):
        r = self.request_class(url=u'http://www.scrapy.org/price/ "',encode_url=False)
        self.assertEqual(r.url, 'http://www.scrapy.org/price/ "')
    ...
    def test_replace(self):
        ...
        r1 = self.request_class("http://www.example.com/'hello'", method='GET')
        hdrs = Headers(r1.headers)
        hdrs[b'key'] = b'value'
        r2 = r1.replace(method="POST", body="New body", headers=hdrs, encode_url=True)
    ...
    def test_replace_noescape(self):
        """Test Request.replace() method"""
        r1 = self.request_class("http://www.example.com/'hello'", method='GET')
        hdrs = Headers(r1.headers)
        hdrs[b'key'] = b'value'
        r2 = r1.replace(method="POST", body="New body", headers=hdrs, encode_url=False)
        self.assertEqual(r1.url, r2.url)
        self.assertEqual((r1.method, r2.method), ("GET", "POST"))
        self.assertEqual((r1.body, r2.body), (b'', b"New body"))
        self.assertEqual((r1.headers, r2.headers), (self.default_headers, hdrs))
